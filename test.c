...
void GenMatrix (){
	...
	int SourceMatLine,SourceMatColumn,k1,k2,FinalMatLine,FinalMatColumn,x1,x2;
	...
	for (int j = 0; j <= 1; j++){
		...
		k1 = rand()%x1+1; //произвольное место для начала области (строки)
		k2 = rand()%x2+1; //произвольное место для начала области	(столбцы)
